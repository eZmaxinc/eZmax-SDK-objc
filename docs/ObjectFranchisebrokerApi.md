...
ObjectFranchisebrokerApi*apiInstance = [[ObjectFranchisebrokerApi alloc] init];

// Retrieve Franchisebrokers and IDs
[apiInstance franchisebrokerGetAutocompleteV2WithSSelector:sSelector
              eFilterActive:eFilterActive
              sQuery:sQuery
              acceptLanguage:acceptLanguage
          completionHandler: ^(FranchisebrokerGetAutocompleteV2Response* output, NSError* error) {
                        if (output) {
                            NSLog(@"%@", output);
                        }
                        if (error) {
                            NSLog(@"Error calling ObjectFranchisebrokerApi->franchisebrokerGetAutocompleteV2: %@", error);
                        }
                    }];
```
