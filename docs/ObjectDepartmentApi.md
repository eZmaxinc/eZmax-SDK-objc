...
ObjectDepartmentApi*apiInstance = [[ObjectDepartmentApi alloc] init];
...
[apiInstance departmentGetAutocompleteV2WithSSelector:sSelector
          ...
          completionHandler: ^(DepartmentGetAutocompleteV2Response* output, NSError* error) {
                        if (output) {
                            NSLog(@"%@", output);
                        }
                        if (error) {
                            NSLog(@"Error calling ObjectDepartmentApi->departmentGetAutocompleteV2: %@", error);
                        }
                    }];
```
