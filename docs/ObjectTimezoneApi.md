...
ObjectTimezoneApi*apiInstance = [[ObjectTimezoneApi alloc] init];
...
[apiInstance timezoneGetAutocompleteV2WithSSelector:sSelector
          ...
          completionHandler: ^(TimezoneGetAutocompleteV2Response* output, NSError* error) {
                        if (output) {
                            NSLog(@"%@", output);
                        }
                        if (error) {
                            NSLog(@"Error calling ObjectTimezoneApi->timezoneGetAutocompleteV2: %@", error);
                        }
                    }];
```
