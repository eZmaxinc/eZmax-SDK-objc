...
NSString* sSelector = @"sSelector_example"; // The type of Modules to return
NSString* eFilterActive = @"Active"; // Specify which results we want to display. (optional) (default to @"Active")
NSString* sQuery = @"sQuery_example"; // Allow to filter the returned results (optional)
HeaderAcceptLanguage acceptLanguage = [[HeaderAcceptLanguage alloc] init]; //  (optional)

ObjectModuleApi*apiInstance = [[ObjectModuleApi alloc] init];

// Retrieve Modules and IDs
[apiInstance moduleGetAutocompleteV2WithSSelector:sSelector
              eFilterActive:eFilterActive
              sQuery:sQuery
              acceptLanguage:acceptLanguage
          completionHandler: ^(ModuleGetAutocompleteV2Response* output, NSError* error) {
                        if (output) {
                            NSLog(@"%@", output);
                        }
                        if (error) {
                            NSLog(@"Error calling ObjectModuleApi->moduleGetAutocompleteV2: %@", error);
                        }
                    }];
```
...
------------- | ------------- | ------------- | -------------
 **sSelector** | **NSString***| The type of Modules to return | 
