#import "ObjectUserlogintypeApi.h"
...
@interface ObjectUserlogintypeApi ()

@property (nonatomic, strong, readwrite) NSMutableDictionary *mutableDefaultHeaders;

@end

@implementation ObjectUserlogintypeApi

NSString* kObjectUserlogintypeApiErrorDomain = @"ObjectUserlogintypeApiErrorDomain";
NSInteger kObjectUserlogintypeApiMissingParamErrorCode = 234513;
...
-(NSURLSessionTask*) userlogintypeGetAutocompleteV2WithSSelector: (NSString*) sSelector
    ...
    if (sSelector == nil) {
        ...
        if(handler) {
            ...
            NSError* error = [NSError errorWithDomain:kObjectUserlogintypeApiErrorDomain code:kObjectUserlogintypeApiMissingParamErrorCode userInfo:userInfo];
            handler(nil, error);
        }
        return nil;
    }

    NSMutableString* resourcePath = [NSMutableString stringWithFormat:@"/2/object/userlogintype/getAutocomplete/{sSelector}"];
    ...
    return [self.apiClient requestWithPath: resourcePath
                       ...
                       responseContentType: responseContentType
                              responseType: @"UserlogintypeGetAutocompleteV2Response*"
                           completionBlock: ^(id data, NSError *error) {
                                if(handler) {
                                    handler((UserlogintypeGetAutocompleteV2Response*)data, error);
                                }
                            }];
}
