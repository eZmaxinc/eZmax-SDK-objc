...
@interface ObjectLanguageApi ()

@property (nonatomic, strong, readwrite) NSMutableDictionary *mutableDefaultHeaders;

@end

@implementation ObjectLanguageApi

NSString* kObjectLanguageApiErrorDomain = @"ObjectLanguageApiErrorDomain";
NSInteger kObjectLanguageApiMissingParamErrorCode = 234513;
...
///
///  @returns LanguageGetAutocompleteV2Response*
///
-(NSURLSessionTask*) languageGetAutocompleteV2WithSSelector: (NSString*) sSelector
    ...
    if (sSelector == nil) {
        ...
        if(handler) {
            ...
            NSError* error = [NSError errorWithDomain:kObjectLanguageApiErrorDomain code:kObjectLanguageApiMissingParamErrorCode userInfo:userInfo];
            handler(nil, error);
        }
        return nil;
    }

    NSMutableString* resourcePath = [NSMutableString stringWithFormat:@"/2/object/language/getAutocomplete/{sSelector}"];
    ...
    return [self.apiClient requestWithPath: resourcePath
                       ...
                       responseContentType: responseContentType
                              responseType: @"LanguageGetAutocompleteV2Response*"
                           completionBlock: ^(id data, NSError *error) {
                                if(handler) {
                                    handler((LanguageGetAutocompleteV2Response*)data, error);
                                }
                            }];
}
