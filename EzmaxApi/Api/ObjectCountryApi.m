...
@interface ObjectCountryApi ()

@property (nonatomic, strong, readwrite) NSMutableDictionary *mutableDefaultHeaders;

@end

@implementation ObjectCountryApi

NSString* kObjectCountryApiErrorDomain = @"ObjectCountryApiErrorDomain";
NSInteger kObjectCountryApiMissingParamErrorCode = 234513;
...
///
///  @returns CountryGetAutocompleteV2Response*
///
-(NSURLSessionTask*) countryGetAutocompleteV2WithSSelector: (NSString*) sSelector
    ...
    if (sSelector == nil) {
        ...
        if(handler) {
            ...
            NSError* error = [NSError errorWithDomain:kObjectCountryApiErrorDomain code:kObjectCountryApiMissingParamErrorCode userInfo:userInfo];
            handler(nil, error);
        }
        return nil;
    }

    NSMutableString* resourcePath = [NSMutableString stringWithFormat:@"/2/object/country/getAutocomplete/{sSelector}"];
    ...
    return [self.apiClient requestWithPath: resourcePath
                       ...
                       responseContentType: responseContentType
                              responseType: @"CountryGetAutocompleteV2Response*"
                           completionBlock: ^(id data, NSError *error) {
                                if(handler) {
                                    handler((CountryGetAutocompleteV2Response*)data, error);
                                }
                            }];
}
