...
@interface ObjectPeriodApi ()

@property (nonatomic, strong, readwrite) NSMutableDictionary *mutableDefaultHeaders;

@end

@implementation ObjectPeriodApi

NSString* kObjectPeriodApiErrorDomain = @"ObjectPeriodApiErrorDomain";
NSInteger kObjectPeriodApiMissingParamErrorCode = 234513;
...
-(NSURLSessionTask*) periodGetAutocompleteV1WithSSelector: (NSString*) sSelector
    ...
    if (sSelector == nil) {
        ...
        if(handler) {
            ...
            NSError* error = [NSError errorWithDomain:kObjectPeriodApiErrorDomain code:kObjectPeriodApiMissingParamErrorCode userInfo:userInfo];
            handler(nil, error);
        }
        return nil;
    }
...
-(NSURLSessionTask*) periodGetAutocompleteV2WithSSelector: (NSString*) sSelector
    ...
    return [self.apiClient requestWithPath: resourcePath
                        ...
                        requestContentType: requestContentType
                       responseContentType: responseContentType
                              responseType: @"PeriodGetAutocompleteV2Response*"
                           completionBlock: ^(id data, NSError *error) {
                                if(handler) {
                                    handler((PeriodGetAutocompleteV2Response*)data, error);
                                }
                            }];
}
