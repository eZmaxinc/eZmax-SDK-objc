...
@interface ObjectProvinceApi ()

@property (nonatomic, strong, readwrite) NSMutableDictionary *mutableDefaultHeaders;

@end

@implementation ObjectProvinceApi

NSString* kObjectProvinceApiErrorDomain = @"ObjectProvinceApiErrorDomain";
NSInteger kObjectProvinceApiMissingParamErrorCode = 234513;
...
///
///  @returns ProvinceGetAutocompleteV2Response*
///
-(NSURLSessionTask*) provinceGetAutocompleteV2WithSSelector: (NSString*) sSelector
    ...
    if (sSelector == nil) {
        ...
        if(handler) {
            ...
            NSError* error = [NSError errorWithDomain:kObjectProvinceApiErrorDomain code:kObjectProvinceApiMissingParamErrorCode userInfo:userInfo];
            handler(nil, error);
        }
        return nil;
    }

    NSMutableString* resourcePath = [NSMutableString stringWithFormat:@"/2/object/province/getAutocomplete/{sSelector}"];
    ...
    return [self.apiClient requestWithPath: resourcePath
                       ...
                       responseContentType: responseContentType
                              responseType: @"ProvinceGetAutocompleteV2Response*"
                           completionBlock: ^(id data, NSError *error) {
                                if(handler) {
                                    handler((ProvinceGetAutocompleteV2Response*)data, error);
                                }
                            }];
}
