...
-(NSURLSessionTask*) invoiceBatchDownloadV1WithPkiInvoiceID: (NSNumber*) pkiInvoiceID
    ...
    if (invoiceBatchDownloadV1Request == nil) {
        NSParameterAssert(invoiceBatchDownloadV1Request);
        if(handler) {
            NSDictionary * userInfo = @{NSLocalizedDescriptionKey : [NSString stringWithFormat:NSLocalizedString(@"Missing required parameter '%@'", nil),@"invoiceBatchDownloadV1Request"] };
            NSError* error = [NSError errorWithDomain:kObjectInvoiceApiErrorDomain code:kObjectInvoiceApiMissingParamErrorCode userInfo:userInfo];
            handler(nil, error);
        }
        return nil;
    ...
    NSMutableString* resourcePath = [NSMutableString stringWithFormat:@"/1/object/invoice/{pkiInvoiceID}/batchDownload"];
    ...
    bodyParam = invoiceBatchDownloadV1Request;
