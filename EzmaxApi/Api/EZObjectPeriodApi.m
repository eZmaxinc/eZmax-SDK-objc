...
@interface EZObjectPeriodApi ()

@property (nonatomic, strong, readwrite) NSMutableDictionary *mutableDefaultHeaders;

@end

@implementation EZObjectPeriodApi

NSString* kEZObjectPeriodApiErrorDomain = @"EZObjectPeriodApiErrorDomain";
NSInteger kEZObjectPeriodApiMissingParamErrorCode = 234513;
...
-(NSURLSessionTask*) periodGetAutocompleteV1WithSSelector: (NSString*) sSelector
    ...
    if (sSelector == nil) {
        ...
        if(handler) {
            ...
            NSError* error = [NSError errorWithDomain:kEZObjectPeriodApiErrorDomain code:kEZObjectPeriodApiMissingParamErrorCode userInfo:userInfo];
            handler(nil, error);
        }
        return nil;
    }

    NSMutableString* resourcePath = [NSMutableString stringWithFormat:@"/1/object/period/getAutocomplete/{sSelector}"];
...
///  @param acceptLanguage  (optional)
///
///  @returns EZPeriodGetAutocompleteV2Response*
///
-(NSURLSessionTask*) periodGetAutocompleteV2WithSSelector: (NSString*) sSelector
    ...
    NSMutableString* resourcePath = [NSMutableString stringWithFormat:@"/2/object/period/getAutocomplete/{sSelector}"];
    ...
    return [self.apiClient requestWithPath: resourcePath
                        ...
                        requestContentType: requestContentType
                       responseContentType: responseContentType
                              responseType: @"EZPeriodGetAutocompleteV2Response*"
                           completionBlock: ^(id data, NSError *error) {
                                if(handler) {
                                    handler((EZPeriodGetAutocompleteV2Response*)data, error);
                                }
                            }];
}
